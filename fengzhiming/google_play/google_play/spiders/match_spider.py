...
class MatchDictSpier(ItemPaddingSpider):
    ...
    def parse(self, response):
        ...
        # 对该条数据进行日志统计
        log_mod.count_data(response.meta['request_id'], data_dict)
        # 获取页面解析后的数据
        for data in data_dict['data']:
            ...
            if data['next_url_request']:
                ...
                yield self.match_dict[url_type]['save'](data, -1)
    ...
    def simple_data_save(self, data):
        ...
        return self.padding_item(data, -1)
    ...
    def get_rule_data(rule_dict, content):
        ...
        temp_item = dict()
        # 正则
        if isinstance(list(rule_dict.values())[0], type(re.compile(r''))):
